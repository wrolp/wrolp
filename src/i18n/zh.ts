...
const zh: Partial<Record<TranslationKey, string>> = {
  ...
  appearanceCardSub: '强调色、密度、界面字号与动效。',
  accentLabel: '强调色',
  accentHelp: '用于按钮、选中项与焦点底色；深浅色主题各自重算对比度。',
  accentCustom: '自定义颜色…',
  accentReset: '恢复默认',
  presetAccentBlue: '蓝',
  presetAccentCyan: '青',
  presetAccentMoss: '苔绿',
  presetAccentAmber: '琥珀',
  presetAccentClay: '陶土',
  presetAccentIris: '鸢尾',
  densityLabel: '密度',
  densityHelp: '控制行高与工具栏高度。切换密度只会带动你没手动拖拽过的导航列与检查器宽度。',
  densityCompact: '紧凑',
  densityComfy: '舒适',
  uiFontSizeLabel: '界面字号',
  uiFontSizeHelp: '目前覆盖设置页、卡片与导航框架；各面板正文仍是固定字号。',
  uiFontSizeNow: '当前：{val}px',
  motionLabel: '动效',
  motionHelp: '「跟随系统」遵从系统的减弱动效设置；「关闭」则无条件停用过渡与动画。',
  motionSystem: '跟随系统',
  motionOn: '开启',
  motionOff: '关闭',
  focusRingLabel: '焦点可见环',
  focusRingHelp: '键盘聚焦时控件外围显示的描边。',
...
无效正则会红框提示且不生效；「恢复默认」不清除自定义规则。`,
  ...
  snippetGroupGeneral: '通用',
  snippetGroupUnknown: '未知连接',
  snippetFilterActiveConnection: '仅当前连接',
  snippetNoCommandsForConnection: '该连接下暂无命令。',
  snippetParamsBadge: '参数与选项',
  snippetConnection: '连接',
  snippetParamSection: '参数',
  snippetParamName: '名称',
  snippetParamType: '类型',
  snippetParamTypeText: '文本',
  snippetParamTypeSelect: '下拉',
  snippetParamDefault: '默认值',
  snippetParamOptions: '选项（逗号或换行分隔）',
  snippetParamOptionsRequired: '下拉参数至少要有一个选项',
  snippetParamDescription: '描述（可选）',
  snippetParamEnabledByDefault: '默认勾选',
  snippetExclusiveGroup: '互斥组（可选）',
  snippetExclusiveGroupHint: '取值相同的选项/参数互斥，勾选一个会自动取消其它',
  snippetLink: '联动',
  snippetLinkHint: '启用本项时自动勾选所选条目（取消勾选不会反向取消）',
  snippetLinkNone: '没有其它条目可联动',
  snippetFillLinkList: '启用时联动：{items}',
  snippetGroupByConnection: '按连接',
  snippetGroupByCustom: '按分组',
  snippetGroupNew: '新建分组',
  snippetGroupNewPlaceholder: '分组名称',
  snippetGroupExists: '该分组已存在',
  snippetGroupRename: '重命名分组',
  snippetGroupRenameMerge: '该名称已存在，重命名后将合并两个分组',
  snippetGroupDelete: '删除分组',
  snippetGroupDeleteConfirm: '删除分组「{name}」？组内命令会移到「未分组」，命令本身不会被删除。',
  snippetGroupEmptyHint: '这个分组还没有命令，右键命令可移动到分组，或拖拽命令到此处。',
  snippetMoveToGroup: '移动到分组',
  snippetParamAdd: '添加参数',
  snippetParamRemove: '删除参数',
  snippetParamOrphan: '命令中未引用',
  snippetParamDuplicate: '参数名重复：{name}',
  snippetParamNameInvalid: '参数名须以字母或下划线开头，只能包含字母、数字和下划线',
  snippetOptionSection: '选项',
  snippetOptionText: '片段',
  snippetOptionLabel: '显示名（可选）',
  snippetOptionValueType: '值',
  snippetOptionValueNone: '无值',
  snippetOptionValueText: '文本',
  snippetOptionValueSelect: '下拉',
  snippetOptionSpaceSeparated: '空格分隔',
  snippetOptionSpaceSeparatedHint: '勾选后用空格连接值（--x v），不勾选用等号（--x=v）',
  snippetOptionValueRequired: '该选项的片段需要内嵌一个 ${...} 值槽',
  snippetOptionOrphan: '命令中未找到该片段',
  snippetOptionDetect: '识别选项',
  snippetOptionAdd: '添加选项',
  snippetOptionRemove: '删除选项',
  snippetOptionMultiValueSlot: '一个选项片段最多只能有一个值占位符',
  snippetFillParamsTitle: '填写参数',
  snippetFillParamsDesc: '发送到终端前，填写取值并勾选要包含的项。',
  snippetFillToggle: '包含',
  snippetFillResetItem: '恢复默认值',
  snippetFillResetAll: '全部恢复默认',
